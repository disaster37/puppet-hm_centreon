    ...
    module APIClient
        class ServiceTemplate
            ...
            def load(service_template)
                ...
                #    service_template.set_description(data["alias"]) unless data["alias"].nil?
                #    service_template.set_is_activated(true) if data["activate"] == 1
                #    service_template.set_is_activated(false) unless data["activate"] == 1
            ...
            def add(service_template, retrive_id = true)
                ...
                @client.post({
                    "action": "add",
                    "object": "stpl",
                    "values": sprintf("%s;%s;%s", service_template.name(), service_template.description(), service_template.template())
                }.to_json)
                ...
                set_param(service_template.name(), "activate", "0") unless service_template.is_activated
                set_param(service_template.name(), "activate", "1") if service_template.is_activated
                ...
                # Get and set id
                if retrive_id
                    service_tmp = get(service_template.name(), true)
            ...
            def update(service_template, macros = true, activated = true, check_command_arguments = true)
                ...
                set_param(service_template.name(), "notes_url", service_template.note_url()) unless service_template.note_url().nil?
                set_param(service_template.name(), "action_url", service_template.action_url()) unless service_template.action_url().nil?
                set_param(service_template.name(), "activate", "0") if !service_template.is_activated && activated
                set_param(service_template.name(), "activate", "1") if service_template.is_activated && activated
            ...
            def set_param(service_template_name, name, value)
                ...
                @client.post({
                    "action": "setparam",
                    "object": "stpl",
                    "values": sprintf("%s;%s;%s", service_template_name, name, value.to_s)
                }.to_json)
            end
            ...
            def get_param(service_template_name, name)
                ...
                r = @client.post({
                    "action": "getparam",
                    "object": "stpl",
                    "values": sprintf("%s;%s", service_template_name, name)
                }.to_json)
            ...
            def set_macro(service_template_name, macro)
                ...
                @client.post({
                    "action": "setmacro",
                    "object": "stpl",
                    "values": sprintf("%s;%s;%s;%s;%s", service_template_name, macro.name().upcase(), macro.value(), is_password, description)
                }.to_json)
            end
