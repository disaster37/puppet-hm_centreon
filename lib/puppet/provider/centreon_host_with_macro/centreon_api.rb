    ...
    if @property_hash[:ensure] != :absent && !@property_flush.empty? && !@property_flush[:macros].nil? && !@property_flush[:macros].empty?
      ...
      macros_to_create = @property_flush[:macros].reject do |macro|
        ...
        @property_hash[:macros].each do |existing_macro|
          next unless macro['name'] == existing_macro['name'] && macro['value'] == existing_macro['value'] && macro['description'] == existing_macro['description'] && macro['is_password'].to_s == existing_macro['is_password'].to_s
          puts 'found'
          is_found = true
          break
        end
